# arcgis-demographic-clustering

Demographic clutering using ArcGIS Business Analyst data.
...
3 - Explore - If you are more into Python, a good place to start is `jupyter lab` from the root of the project, and look in the `./notebooks` directory. If GIS is more your schtick, open the project `./arcgis/arcgis-demographic-clustering.aprx`.
...
- `make env` - builds the Conda environment with all the name and dependencies from `environment_dev.yml` and installs the local project package `arcgis_clustering` using the command `python -m pip install -e ./src/src/arcgis_clustering` so you can easily test against the package as you are developing it.
